...
pub enum VerifiedType<'a> {
    Contract(subjugated::club::Contract<'a>),
    PartialContract(subjugated::club::PartialContract<'a>),
    NoVerifiedType
}
...
impl ContractVerifier {
    pub fn verify(self, incoming_data: &Vec<u8>) -> Result<VerifiedType, String> {
        match subjugated::club::root_as_signed_message(incoming_data.as_slice()) {
            Ok(signed_msg) => {
                log::debug!("Signed message: {:?}", signed_msg);
                                        ...
                                        Ok(valid_signature) => {
                                            ...
                                            match vkey.verify(&hash, &valid_signature) {
                                                Ok(_) => {
                                                    ...
                                                    return Ok(VerifiedType::PartialContract(partial_contract));
                                                }
                                                Err(e) => {
